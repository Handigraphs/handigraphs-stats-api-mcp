...
```console
gh release create v0.2.5 --repo Handigraphs/handigraphs-stats-api-mcp --target main --title "v0.2.5" --generate-notes
```

This creates the `v0.2.5` tag and triggers `.github/workflows/publish.yml`. Do not run `npm publish` locally for the normal release path.

Watch the workflow in GitHub Actions, then verify both distributions:

```console
npm view @handigraphs/stats-api-mcp version
gh release view v0.2.5 --repo Handigraphs/handigraphs-stats-api-mcp
```

The npm result must be `0.2.5`, and the GitHub Release must include `handigraphs-stats-api-mcp-0.2.5.mcpb`.
